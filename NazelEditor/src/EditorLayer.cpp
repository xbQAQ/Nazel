...
void EditorLayer::OnUpdate(TimeStep deltaTime){
	...
	Nazel::Renderer2D::ResetStats();
	{
		...
		m_Framebuffer->Bind();
		Nazel::RenderCommand::SetClearColor({ 0.1f, 0.1f, 0.1f, 1 });
...

	{
		...
		Nazel::Renderer2D::DrawQuad({ 0.0f, 0.0f, -0.1f }, { 20.0f, 20.0f }, m_CheckerboardTexture, 10.0f);
	...
	if (dockingEnabled){
		...
		if (ImGui::BeginMenuBar()){
			if (ImGui::BeginMenu("File")){
				...
				if (ImGui::MenuItem("Exit")) Nazel::Application::Get().Close();
				ImGui::EndMenu();
			}
